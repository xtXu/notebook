...
思路：定义`dp[i]`为以`nums[i]`结尾的最长递增子序列的长度。则对于`dp[i]`，若存在小于`i`的`j`使得`nums[i]>nums[j]`，则此时`nums[i]`可接在以`nums[j]`为结尾的最长递增子序列后，长度为`dp[j]+1`；遍历`[0,i-1]`，得到最大的长度`dp[i]=max(dp[i],dp[j]+1)`。  
代码：
```cpp
class Solution {
public:
