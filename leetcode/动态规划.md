...
+ 当`s[i-1]!=t[j-1]`时，存在`s[i-1]`的子序列一定以`s[i-1]`结尾，因此不可能与`t[0,j-1]`相同，此时`t[0,j-1]`出现的次数等于在`s[0,i-2]`的子序列中出现的次数，即`dp[i-1][j]`。当`s[i-1]==t[j-1]`时，`t[0,j-1]`除了在`s[0,i-2]`的子序列中出现，还可能是在`s[0,i-2]`中出现的`t[0,j-2]`拼上`s[i-1]`。
代码：
```cpp
