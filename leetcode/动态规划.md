...
3. 初始化`dp[0]=1`。
4. 由于是求组合，因此先遍历物品再遍历背包，即第`i`层的`dp[j-nums[i]]`代表最后一个物品为`i`时用`[0,i]`装满背包的组合个数，而`dp[j]`就代表用`[0,i]`装满背包的组合个数。  
...
4. 由于是求排列，因此先遍历背包再遍历物品，即考虑背包容量为`j`时考虑物品`i`，此时`dp[j-nums[i]]`代表最后一个物品为`i`时的组合个数，`i`之前的物品可以从所有物品中挑选。  
