...
思路：`gas[i]-cost[i]`可得从`i`开到`i+1`的汽油净消耗`rest[i]` 。若`rest[i]`的总和大于等于0，则说明存在解。此时，从`0`开始，累加`rest[i]`为`curSum`。（1）假设加到`k`时，`curSum<0`，说明从`[0,k]`中的任一点出发，到达`k`时一定会使燃油耗尽，起始位置只能从`k+1`开始，`curSum`重置为0。（2）当从某一位置`l`开始，到末尾的累加小于0，则说明`l`一定是可行解。 
证明：（1）假设存在`j`在`[0,k)`之间使得`[j,k]`的累加大于0，由于`[0,k]`的累加小于0，因此`[0,j]`的累加小于0，这与到`k`时`curSum`才小于0矛盾。（2）从`l`开始，则`[0,l-1]`不论分了几段，累加一定小于0。当存在解时，`rest[i]`的总和大于等于0，因此`[l,n-1]`的累加的绝对值一定大于`[0,l-1]`的累加绝对值，
代码：
```cpp
class Solution {
public:
