...
思路：首先按身高`hi`由高到低排序，身高相同则按`ki`由低到高排序。这样，每个人的前面都是身高大于等于自己的人，且同身高的人的相对顺序正确（身高相同，若排在后面，则前面满足要求的人肯定多，即`ki`大。然后，从前往后，将每个人插入`ki`号位置。由于每个人的前面都是身高大于等于自己的人，因此插入`ki`号位置后，前面正好有`ki`个满足条件的人。且从前往后身高由高到低，那么身高低的往前插入时，不会影响身高高的人前面满足要求的人数。  
代码：`vector`法
```cpp
class Solution {
public:
